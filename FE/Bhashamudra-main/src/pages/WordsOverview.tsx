...
const WordOverview = () => {
  ...
  const completedWords = 3; // Example value - replace with actual progress
  const progressPercentage = (completedWords / totalWords) * 100;
  
  // Add completion status to each word (mock data)
  const wordsWithStatus = words.map((word, index) => ({
    word,
    isCompleted: index < completedWords,
  }));
  ...
  return (
    <div className="min-h-screen bg-gradient-to-r from-[#98E7DE] to-[#ffe8e1]">
      <div className="max-w-7xl mx-auto px-4 py-8">
        ...
        <div className="text-center mb-8">
          ...
          <div className="max-w-2xl mx-auto mb-12">
            ...
            <div className="text-sm text-[#004748]/70 mt-2">
              {completedWords} of {totalWords} words mastered
            </div>
          </div>
        </div>
        ...
        >
          {wordsWithStatus.map(({ word, isCompleted }) => (
            <motion.div
              key={word}
            ...
            >
              <button
                onClick={() => navigate(`/learn/word/${word}`)}
              ...
              >
                <div className="absolute inset-0 flex items-center justify-center">
                  <span className={`text-xl sm:text-2xl font-medium transition-transform text-center px-2
